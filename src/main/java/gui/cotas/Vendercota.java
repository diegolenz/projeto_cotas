...
import lib.dao.imp.cotas.PessoaCotaTitularDao;
...
public class Vendercota extends javax.swing.JInternalFrame {
    ...
    private PeriodoCotaDao periodoCotaDao=new PeriodoCotaDao();
    private Cota novacota = new Cota();
    private ModeloPeriodo cotamodelo = new ModeloPeriodo();
    private List<ModeloPeriodo> cotas = new ArrayList<>();
    private JFrame tela;
    ...
    public Vendercota() {
        ...
        initComponents();
        cotas = modeloPeriodoDao.listar();
        for (ModeloPeriodo c : cotas) {
    ...
    private void initComponents() {
        ...
        edtvalormensalidade.setEnabled(false);
    ...
    private void btnpesqapartamentoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnpesqapartamentoActionPerformed
        SelecionaCota.Callback callback = (apartamento) -> {
            ...
            this.edthotel.setText(apartamento.getBloco().getHotel().getNome());
            this.edtbloco.setText((apartamento.getBloco().getDescricao()));
            this.edtapartamento.setText(String.valueOf(apartamento.getNumero()));
    ...
    private void btnconfirmarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnconfirmarActionPerformed
        modeloPeriodoDao.closeConnection();
        CotasDao cotasDao = new CotasDao();
        PessoaCotaTitularDao pessoaCotaTitularDao = new PessoaCotaTitularDao();
        for (Periodo p : cotamodelo.getPeriodos()) {
            periodos.add(p);
        }
        novacota.setDescricao(cotamodelo.getDescricao());
        novacota.setValor(cotamodelo.getValor());
        novacota.setValormensalidade(cotamodelo.getValormensalidade());
        novacota.setIntegrantes(pessoascota);
        novacota.setTitular(pessoatitular);
        novacota.setApartamento(this.apartamento);
        novacota.setSituacao(statuscota.ATIVADO.name());
        try {
            pessoaCotaTitularDao.Inserir(pessoatitular);
    ...
    public boolean verrificatitular() {
        ...
        } else if (novacota.getTitular() != null) {
            if (pessoatitular.getPessoa().getNome() == novacota.getTitular().getPessoa().getNome()
                    && (pessoatitular.getPessoa().getCpfCnpj() == novacota.getTitular().getPessoa().getCpfCnpj())) {
    ...
    private void btnremoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnremoverActionPerformed
        if (this.tabelaagregadostitulo.getSelectedRow() < 0) {
            ...
            if ((novacota.getTitular() != null)&&(pessoacota.getNome() == novacota.getTitular().getPessoa().getNome())
                        && (pessoacota.getCpfCnpj() == novacota.getTitular().getPessoa().getCpfCnpj())) {
    ...
    private void cmbcotaActionPerformed(java.awt.event.ActionEvent evt) {
        if  ( this.cmbcota.getSelectedItem()!= null){
            cotamodelo=cotas.get(this.cmbcota.getSelectedIndex());
            edtvalor.setText(cotamodelo.getValor().toString());
            edtvalormensalidade.setText(cotamodelo.getValormensalidade().toString());
        }
    }
